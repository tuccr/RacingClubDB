...
-- 4a. All drivers currently signed to a specific team
...
-- 4b. All races held in a specific country
...
-- 4c. All vehicles currently driven by a specific driver
SELECT v.vid, v.brand, v.model, v.license_num
FROM Vehicles v
WHERE v.driverid = 'D001';

-- 4d. Drivers with a win rate of 50% or better
...
-- 5a. Top 5 drivers by total wins
SELECT driverid, dwins, dlosses
FROM Drivers
ORDER BY dwins DESC
LIMIT 5;

-- 5b. Team leaderboard - wins, losses, win %, total payroll
SELECT
    tm.teamid,
    tm.tname,
    tm.twins,
...
-- 5c. Most-used tracks (by race count) with average capacity
SELECT
    t.trackid,
    t.trname,
    t.trcity,
...
-- 5d. Race winners report - driver, team at the time, track, date
SELECT
    r.raceid,
...
-- 5e. Free agents - drivers with no active contract
...
-- 5f. Per-driver, per-track performance breakdown
SELECT
    rr.driverid,
    t.trname,
    COUNT(*)                                      AS races_run,
